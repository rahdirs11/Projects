...
from todo.forms import RegistrationForm, LoginForm, AddListForm, AddTaskForm, RenameTaskForm, RenameListForm
...
def deleteList(lid):
    ...
    if lid in range(1, len(lists) + 1):
        toBeDeleted = Lists.query.filter_by(Id=lists[lid - 1].lid).first()
        db.session.delete(toBeDeleted)
        db.session.commit()
        flash(f'Successfully deleted List#{lid}', 'success')
...
def addTask(ID):
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            flash(f'Added a new task to List#{ID}', 'success')
